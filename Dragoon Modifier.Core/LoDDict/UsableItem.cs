...
namespace Dragoon_Modifier.Core.LoDDict {
    public class UsableItem : Item {
        ...
        private string _battleDescription = "<END>";
        private string _encodedBattleDescription = "FF A0 FF A0";
        private byte _target = 0xC0;
        private byte _element = 0;
        private byte _damage = 0;
        private byte _special1 = 0;
        private byte _special2 = 0;
        private byte _unknown1 = 0;
        private byte _specialAmmount = 0;
        private byte _status = 0;
        private byte _percentage = 0;
        private byte _unknown2 = 0;
        private byte _baseSwitch = 0;
        ...
        internal UsableItem(byte index, string[] values) {
            ...
            if (LoDDictionary.Element2Num.TryGetValue(values[2].ToLower(), out bkey)) {
                _element = bkey;
            } else {
                error.Add($"{values[2]} not found as element.");
            }
            if (LoDDictionary.DamageBase2Num.TryGetValue(values[3], out bkey)) {
                _damage = bkey;
            } else if (values[3] != "") {
            ...
            foreach (string substring in values[9].Replace(" ", "").Split(',')) {
                if (LoDDictionary.Status2Num.TryGetValue(substring.ToLower(), out bkey)) {
                    _status |= bkey;
                } else {
                    errorTemp.Add(substring);
                }
            ...
            if (error.Count > 0) {
                Output.WriteError($"Item: {Name} - ID: {Id}");
