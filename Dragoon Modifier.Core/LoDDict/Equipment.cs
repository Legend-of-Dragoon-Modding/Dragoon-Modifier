...
namespace Dragoon_Modifier.Core.LoDDict {
    public class Equipment : Item {
        ...
        public Equipment(byte index, string[] values) {
            ...
            if (LoDDictionary.Element2Num.TryGetValue(values[4].ToLower(), out bkey)) {
                _weaponElement = bkey;
            } else {
                error.Add($"{values[4]} not found as weapon element.");
            }

            if (LoDDictionary.Status2Num.TryGetValue(values[5].ToLower(), out bkey)) {
                _onHitStatus = bkey;
            } else {
            ...
            foreach (string substring in values[16].Replace(" ", "").Split(',')) {
                if (LoDDictionary.Element2Num.TryGetValue(substring.ToLower(), out bkey)) {
