...
namespace Dragoon_Modifier.Core {
    internal static class KMP {
        ...
        private static readonly Dictionary<string, byte[]> Mask = new Dictionary<string, byte[]>() {
            {"??", new byte[] { 0x00, 0x00} },
        ...
        internal static List<long> Search(string pattern, byte[] array, bool findAll = false) {
            ...
            for (int i = 0; i < splitString.Length; i++) {
                if (Byte.TryParse(splitString[i], NumberStyles.HexNumber, null, out byte key)) {
                    ...
                    patternValue[i] = Mask[splitString[i]][0]; // For masked nibbles, value and mask is set to 0, so it always passes
                    patternMask[i] = Mask[splitString[i]][1];
                }
            }
