...
namespace Dragoon_Modifier.DraMod.Dataset.Scripts.HellMode.Characters {
    internal class Lavitz : ICharacter {
        ...
        public void BattleSetup(byte slot) {
            ...
            if (battleTable.ID == 1) {
                ...
                switch (dlv) {
                    ...
                    case 5:
                    case 6:
                        Emulator.DirectAccess.WriteAoB(Emulator.GetAddress("DRAGOON_SPELL_SLOT") + slot * 0x9 + 1, "0E 1A 11 20 1A FF FF FF");
                        break;
                    case 7:
                        Emulator.DirectAccess.WriteAoB(Emulator.GetAddress("DRAGOON_SPELL_SLOT") + slot * 0x9 + 1, "0E 1A 11 20 1A 21 FF FF");
                        break;
                }
            }
        ...
        private void WriteMP(Core.Memory.Battle.Character battleTable) {
            Core.Memory.Battle.DragoonSpell lavitzFlowerStorm = new Core.Memory.Battle.DragoonSpell(7);
            Core.Memory.Battle.DragoonSpell albertFlowerStorm = new Core.Memory.Battle.DragoonSpell(26);
            Core.Memory.Battle.DragoonSpell spinningGale = new Core.Memory.Battle.DragoonSpell(32);
            Core.Memory.Battle.DragoonSpell windAttack = new Core.Memory.Battle.DragoonSpell(33);
            Dictionary<int, IDragoonSpells> datasetSpell = Settings.Instance.Dataset.DragoonSpell;
            lavitzFlowerStorm.MP = (byte) ((DraMod.Settings.Instance.FlowerStorm + 2) * 20);
            albertFlowerStorm.MP = (byte) ((DraMod.Settings.Instance.FlowerStorm + 2) * 20);

            Emulator.DirectAccess.WriteText(datasetSpell[7].Description_Pointer + 44, (DraMod.Settings.Instance.FlowerStorm + 2).ToString());
            Emulator.DirectAccess.WriteText(datasetSpell[26].Description_Pointer + 44, (DraMod.Settings.Instance.FlowerStorm + 2).ToString());
