...
public class BattleController {
    public static void Run(Emulator emulator) {
        ...
        if (Globals.IN_BATTLE && Globals.STATS_CHANGED && encounterValue == 41215 && Globals.PARTY_SLOT[0] == 4 && emulator.ReadByte(Constants.GetAddress("HASCHEL_FIX" + Globals.DISC)) != 0x80) {
            ...
            emulator.WriteAOB(Constants.GetAddress("HASCHEL_FIX" + Globals.DISC), ("0x1E 0x80 0x74 0x12 0x00 0x00 0x02 0x00 0x8C 0x8C 0x4D 0x52 0x47 0x1A 0x04 0x00 0x00 0x00 0x28 0x00" +
            ...
            if (Globals.STATS_CHANGED && encounterValue < 9999) {
                ...
                if (Globals.ADDITION_CHANGE == true) {
                    ...
                    for (int addition = 0; addition < 35; addition++) {
                        ...
                        emulator.WriteShortU(address + addition * 0xE + 0x2, sp1);
                        emulator.WriteShortU(address + addition * 0xE + 0x4, sp2);
                        emulator.WriteShortU(address + addition * 0xE + 0x6, sp3);
                        emulator.WriteShortU(address + addition * 0xE + 0x8, sp4);
    ...
    public static void ChangeParty(Emulator emulator) {
        ...
        if (Globals.NO_DART != null) {
            ...
            if (Globals.NO_DART == 4) {
                Constants.WriteDebug("Haschel Fix");
                emulator.WriteAOB(Constants.GetAddress("HASCHEL_FIX" + Globals.DISC), ("0x80 0x80 0x80 0x00 0x90 0xA0 0x1E 0x80 0x74 0x12 0x00 0x00 0x02 0x00 0x8C 0x8C 0x4D 0x52 0x47 0x1A 0x04 0x00 0x00 0x00 0x28 0x00" +
                    " 0x00 0x00 0x02 0x00 0x00 0x00 0x2C 0x00 0x00 0x00 0x68 0x00 0x00 0x00 0x94 0x00 0x00 0x00 0xD4 0x11 0x00 0x00 0x68 0x12 0x00 0x00 0xB0 0x05 0x02 0x00 0x00 0x00 0x8C 0x8C 0x00 0x00" +
                    " 0x00 0x01 0x00 0x02 0x00 0x03 0x00 0x04 0x00 0x05 0x00 0x06 0x00 0x07 0x00 0x08 0x00 0x09 0x00 0x0A 0x00 0x0B 0x00 0x0C 0x00 0x0D 0x00 0x0E 0x00 0x0F 0x00 0x10 0x00 0x11 0x00 0x12" +
                    " 0x00 0x13 0x00 0x14 0x00 0x15 0x00 0x16 0x00 0x17 0x00 0x18 0x00 0x19 0x00 0x1A 0x00 0x1B"));
            }
