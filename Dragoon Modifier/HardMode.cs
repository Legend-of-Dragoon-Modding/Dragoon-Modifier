    ...
    public static class HardMode {
        ...
        public static void Setup(string difficulty) {
            ...
            if (Globals.PARTY_SLOT.Contains((byte) 2)) {
                ...
                starChildren = 0;
                recoveryRateSave = Emulator.Battle.CharacterTable[index].HP_Regen;
        ...
        static void EquipChangesSetup(string difficulty) { // TODO remove Emulator
            ...
            if (Globals.PARTY_SLOT[0] == 2 || Globals.PARTY_SLOT[0] == 8) {
                shanaSaveAT = Emulator.Battle.CharacterTable[0].AT;
            }
        ...
        static void Chapter3Buffs() {
            for (int i = 0; i < Emulator.Battle.CharacterTable.Length; i++) {
                if (Globals.CHAPTER >= 3) {
                    switch (Emulator.Battle.CharacterTable[i].Weapon) {
                        case 28: // Sparkle Arrow
                            var at = (ushort) (Emulator.Battle.CharacterTable[i].AT + sparkleArrowBuff);
                            Emulator.Battle.CharacterTable[i].AT = at;
                            Emulator.Battle.CharacterTable[i].OG_AT = at;
                            break;
                        case 2: // Heat Blade
                            at = (ushort) (Emulator.Battle.CharacterTable[i].AT + heatBladeBuff);
                            Emulator.Battle.CharacterTable[i].AT = at;
                            Emulator.Battle.CharacterTable[i].OG_AT = at;
                            break;
                        case 14: // Shadow Cutter
                            at = (ushort) (Emulator.Battle.CharacterTable[i].AT + shadowCutterBuff);
                            Emulator.Battle.CharacterTable[i].AT = at;
                            Emulator.Battle.CharacterTable[i].OG_AT = at;
                            break;
                        case 35: // Morning Star
                            at = (ushort) (Emulator.Battle.CharacterTable[i].AT + morningStarBuff);
                            Emulator.Battle.CharacterTable[i].AT = at;
                            Emulator.Battle.CharacterTable[i].OG_AT = at;
                            break;
                    }
        ...
        static void PercentageBuffs() {
            for (int slot = 0; slot < Emulator.Battle.CharacterTable.Length; slot++) {
                var level = Emulator.Memory.CharacterTable[Globals.PARTY_SLOT[slot]].Level;

                switch (Globals.PARTY_SLOT[slot]) {
                    case 2: // Shana
                    case 8: // Miranda
                        double boost = 1;
                        if (Emulator.Battle.CharacterTable[slot].Weapon == 32) {
                            boost = 1.4;
                        } else if (level >= 28) {
                            boost = 2.15;
                        } else if (level >= 20) {
                        ...
                        var at = (ushort) (Math.Round(Emulator.Battle.CharacterTable[slot].AT * boost));
                        Emulator.Battle.CharacterTable[slot].AT = at;
                        Emulator.Battle.CharacterTable[slot].OG_AT = at;

                        if (level >= 30) {
                            var df = (ushort) (Math.Round(Emulator.Battle.CharacterTable[slot].DF * 1.12));
                            Emulator.Battle.CharacterTable[slot].DF = df;
                            Emulator.Battle.CharacterTable[slot].OG_DF = df;
                        }
                        break;
                    case 3: // Rose
                        if (level >= 30) {
                            var df = (ushort) (Math.Round(Emulator.Battle.CharacterTable[slot].DF * 1.11));
                            Emulator.Battle.CharacterTable[slot].DF = df;
                            Emulator.Battle.CharacterTable[slot].OG_DF = df;
                        }
                        break;
                    case 6: // Meru
                        if (level >= 30) {
                            var df = (ushort) (Math.Round(Emulator.Battle.CharacterTable[slot].DF * 1.26));
                            Emulator.Battle.CharacterTable[slot].DF = df;
                            Emulator.Battle.CharacterTable[slot].OG_DF = df;
                        }
                        break;
        ...
        static void EquipSetup() {
            ...
            for (int slot = 0; slot < Emulator.Battle.CharacterTable.Length; slot++) {
                switch (Emulator.Battle.CharacterTable[slot].Accessory) {
                    case 149: // Phantom Shield
                        var df = Emulator.Battle.CharacterTable[slot].DF;

                        if (Emulator.Battle.CharacterTable[slot].Armor == 74) {
                            Emulator.Battle.CharacterTable[slot].DF = (ushort) Math.Ceiling(df * 1.1);
                            Emulator.Battle.CharacterTable[slot].OG_DF = (ushort) Math.Ceiling(df * 1.1);
                        } else {
                            Emulator.Battle.CharacterTable[slot].DF = (ushort) Math.Ceiling(df * 0.7);
                            Emulator.Battle.CharacterTable[slot].OG_DF = (ushort) Math.Ceiling(df * 0.7);
                        }

                        var mdf = Emulator.Battle.CharacterTable[slot].MDF;

                        if (Emulator.Battle.CharacterTable[slot].Helmet == 89) {
                            Emulator.Battle.CharacterTable[slot].MDF = (ushort) Math.Ceiling(mdf * 1.1);
                            Emulator.Battle.CharacterTable[slot].OG_MDF = (ushort) Math.Ceiling(mdf * 1.1);
                        } else {
                            Emulator.Battle.CharacterTable[slot].MDF = (ushort) Math.Ceiling(mdf * 0.7);
                            Emulator.Battle.CharacterTable[slot].OG_MDF = (ushort) Math.Ceiling(mdf * 0.7);
                        }
                        break;

                    case 150: // Dragon Shield
                        df = Emulator.Battle.CharacterTable[slot].DF;
                        if (Emulator.Battle.CharacterTable[slot].Armor == 74) {
                            Emulator.Battle.CharacterTable[slot].DF = (ushort) Math.Ceiling(df * 1.2);
                            Emulator.Battle.CharacterTable[slot].OG_DF = (ushort) Math.Ceiling(df * 1.2);
                        } else {
                            Emulator.Battle.CharacterTable[slot].DF = (ushort) Math.Ceiling(df * 0.7);
                            Emulator.Battle.CharacterTable[slot].OG_DF = (ushort) Math.Ceiling(df * 0.7);
                        }
                    ...
                    case 151: // Angel Scarf
                        mdf = Emulator.Battle.CharacterTable[slot].MDF;
                        if (Emulator.Battle.CharacterTable[slot].Helmet == 89) {
                            Emulator.Battle.CharacterTable[slot].MDF = (ushort) Math.Ceiling(mdf * 1.2);
                            Emulator.Battle.CharacterTable[slot].OG_MDF = (ushort) Math.Ceiling(mdf * 1.2);
                        } else {
                            Emulator.Battle.CharacterTable[slot].MDF = (ushort) Math.Ceiling(mdf * 0.7);
                            Emulator.Battle.CharacterTable[slot].OG_MDF = (ushort) Math.Ceiling(mdf * 0.7);
                        }
                    ...
                    case 176: // Soa's Sash
                        for (int i = 0; i < Emulator.Battle.CharacterTable.Length; i++) {
                            if (i != slot) {
                                Emulator.Battle.CharacterTable[i].SP_Multi = (short) (Emulator.Battle.CharacterTable[i].SP_Multi - 50);
                            }
                        }
                        break;
                    ...
                    case 177: // Soa's Ankh
                        if (Globals.CheckDMScript("btnSoloMode")) {
                            Emulator.Battle.CharacterTable[slot].Revive = (byte) (Emulator.Battle.CharacterTable[slot].Revive - 50);
                        }
                    ...
                    case 178: // Soas's Health Ring
                        for (int i = 0; i < Emulator.Battle.CharacterTable.Length; i++) {
                            if (i != slot) {
                                Emulator.Battle.CharacterTable[i].Max_HP = (ushort) Math.Round(Emulator.Battle.CharacterTable[i].Max_HP * 0.75); // TODO simplify to reduce memory reads

                                if (Emulator.Battle.CharacterTable[i].HP > Emulator.Battle.CharacterTable[slot].Max_HP) {
                                    Emulator.Battle.CharacterTable[i].HP = Emulator.Battle.CharacterTable[slot].Max_HP;
                                }
                            }
                        }
                        break;
                    ...
                    case 179: // Soa's Mage Ring
                        for (int i = 0; i < Emulator.Battle.CharacterTable.Length; i++) {
                            if (i != slot) {
                                Emulator.Battle.CharacterTable[i].Max_MP = (ushort) Math.Round(Emulator.Battle.CharacterTable[i].Max_MP * 0.75); // TODO simplify to reduce memory reads

                                if (Emulator.Battle.CharacterTable[i].MP > Emulator.Battle.CharacterTable[slot].Max_MP) {
                                    Emulator.Battle.CharacterTable[i].MP = Emulator.Battle.CharacterTable[slot].Max_MP;
                                }
                            }
                        }
                        break;
                    ...
                    case 181: // Soa's Syphon Ring
                        ...
                        ushort mat = (ushort) (Emulator.Battle.CharacterTable[slot].MAT * 2);
                        Emulator.Battle.CharacterTable[slot].MAT = mat;
                        Emulator.Battle.CharacterTable[slot].OG_MAT = mat;
                        Emulator.Battle.CharacterTable[slot].DMAT = (ushort) Math.Round(Emulator.Battle.CharacterTable[slot].DMAT * 0.3);
                        for (int i = 0; i < Emulator.Battle.CharacterTable.Length; i++) {
                            if (i != slot) {
                                Emulator.Battle.CharacterTable[i].MAT = (ushort) Math.Round(Emulator.Battle.CharacterTable[i].MAT * 0.8);
                                Emulator.Battle.CharacterTable[i].OG_MAT = Emulator.Battle.CharacterTable[i].MAT;
                            }
                        }
                        break;

                    case 130: // Holy Ankh + Angel Robe nerf
                        if (Emulator.Battle.CharacterTable[slot].Armor == 73) { // Angel Robe
                            Emulator.Battle.CharacterTable[slot].Revive = (byte) (Emulator.Battle.CharacterTable[slot].Revive - 20);
                        }
                        break;

                    case 180: // Soas's Shield Ring
                        Emulator.Battle.CharacterTable[slot].HP = 1;
                        Emulator.Battle.CharacterTable[slot].Max_HP = 1;
                        Emulator.Battle.CharacterTable[slot].DF = 10;
                        Emulator.Battle.CharacterTable[slot].MDF = 10;
                        Emulator.Battle.CharacterTable[slot].A_AV = 90;
                        Emulator.Battle.CharacterTable[slot].M_AV = 90;
                        for (int i = 0; i < Emulator.Battle.CharacterTable.Length; i++) {
                            if (i != slot) {
                                Emulator.Battle.CharacterTable[i].A_HIT = (byte) Math.Round(Emulator.Battle.CharacterTable[i].A_HIT * 0.8);
                                Emulator.Battle.CharacterTable[i].M_HIT = (byte) Math.Round(Emulator.Battle.CharacterTable[i].M_HIT * 0.8);
                            }
                        }
                        break;
                }

                switch (Emulator.Battle.CharacterTable[slot].Weapon) {
                    ...
                    case 161: // Element Arrow
                        ...
                        elementArrowTurns = 0;
                        Emulator.Battle.CharacterTable[slot].Weapon_Element = elementArrowElement;
                        break;
                    ...
                    case 166: // Soa's Light
                        Emulator.Battle.CharacterTable[slot].SP_Multi = -100;
                        Emulator.Battle.CharacterTable[slot].SP_Regen = 100;
                        for (int i = 0; i < Emulator.Battle.CharacterTable.Length; i++) {
                            if (i != slot) {
                                ushort df = (ushort) Math.Round(Emulator.Battle.CharacterTable[slot].DF * 0.7);
                                Emulator.Battle.CharacterTable[i].DF = df;
                                Emulator.Battle.CharacterTable[i].OG_DF = df;
                                ushort mdf = (ushort) Math.Round(Emulator.Battle.CharacterTable[slot].MDF * 0.7);
                ...
                switch (Emulator.Battle.CharacterTable[slot].Helmet) {
                    case 168: // Soa's Helm
                        for (int i = 0; i < Emulator.Battle.CharacterTable.Length; i++) {
                            if (i != slot) {
                                ushort at = (ushort) Math.Round(Emulator.Battle.CharacterTable[i].AT * 0.7);
                                Emulator.Battle.CharacterTable[i].AT = at;
                                Emulator.Battle.CharacterTable[i].OG_AT = at;
                            }
                        }
                ...
                switch (Emulator.Battle.CharacterTable[slot].Armor) {
                    case 170: // Divine DG armor
                        var character = Globals.PARTY_SLOT[slot];
                        short sp_p_hit = (short) (Emulator.Battle.CharacterTable[slot].SP_P_Hit + 20);
                        short sp_m_hit = (short) (Emulator.Battle.CharacterTable[slot].SP_M_Hit + 20);
                        short mp_p_hit = (short) (Emulator.Battle.CharacterTable[slot].MP_P_Hit + 10);
                        short mp_m_hit = (short) (Emulator.Battle.CharacterTable[slot].MP_M_Hit + 10);

                        Emulator.Battle.CharacterTable[slot].SP_P_Hit = sp_p_hit;
                        Emulator.Memory.SecondaryCharacterTable[character].SP_P_Hit = sp_p_hit;
                        Emulator.Battle.CharacterTable[slot].SP_M_Hit = sp_m_hit;
                        Emulator.Memory.SecondaryCharacterTable[character].SP_M_Hit = sp_m_hit;
                        Emulator.Battle.CharacterTable[slot].MP_P_Hit = mp_p_hit;
                        Emulator.Memory.SecondaryCharacterTable[character].MP_P_Hit = mp_p_hit;
                        Emulator.Battle.CharacterTable[slot].MP_M_Hit = mp_m_hit;
                        Emulator.Memory.SecondaryCharacterTable[character].MP_M_Hit = mp_m_hit;
                        break;

                    case 171: // Soa's Armor
                        for (int i = 0; i < Emulator.Battle.CharacterTable.Length; i++) {
                            if (i != slot) {
                                ushort mat = (ushort) Math.Round(Emulator.Battle.CharacterTable[i].MAT * 0.7);
                                Emulator.Battle.CharacterTable[i].MAT = mat;
                                Emulator.Battle.CharacterTable[i].OG_MAT = mat;
                            }
                        }
                        break;
                    ...
                    case 74: // Armor of Legend
                        switch (Globals.PARTY_SLOT[slot]) {
                            case 0: // Dart
                                Emulator.Battle.CharacterTable[slot].DF = (ushort) (Emulator.Battle.CharacterTable[slot].DF + 41 - 127);
                                Emulator.Battle.CharacterTable[slot].MDF = (ushort) (Emulator.Battle.CharacterTable[slot].MDF + 40);
                                break;

                            case 1: // Lavitz
                            case 5: // Albert
                                Emulator.Battle.CharacterTable[slot].DF = (ushort) (Emulator.Battle.CharacterTable[slot].DF + 54 - 127);
                                Emulator.Battle.CharacterTable[slot].MDF = (ushort) (Emulator.Battle.CharacterTable[slot].MDF + 27);
                                break;

                            case 2: // Shana
                            case 8: // Miranda
                                Emulator.Battle.CharacterTable[slot].DF = (ushort) (Emulator.Battle.CharacterTable[slot].DF + 27 - 127);
                                Emulator.Battle.CharacterTable[slot].MDF = (ushort) (Emulator.Battle.CharacterTable[slot].MDF + 80);
                                break;

                            case 3: // Rose
                                Emulator.Battle.CharacterTable[slot].DF = (ushort) (Emulator.Battle.CharacterTable[slot].DF + 41 - 127);
                                Emulator.Battle.CharacterTable[slot].MDF = (ushort) (Emulator.Battle.CharacterTable[slot].MDF + 42);
                                break;

                            case 4: // Haschel
                                Emulator.Battle.CharacterTable[slot].DF = (ushort) (Emulator.Battle.CharacterTable[slot].DF + 45 - 127);
                                Emulator.Battle.CharacterTable[slot].MDF = (ushort) (Emulator.Battle.CharacterTable[slot].MDF + 40);
                                break;

                            case 6: // Meru
                                Emulator.Battle.CharacterTable[slot].DF = (ushort) (Emulator.Battle.CharacterTable[slot].DF + 30 - 127);
                                Emulator.Battle.CharacterTable[slot].MDF = (ushort) (Emulator.Battle.CharacterTable[slot].MDF + 54);
                                break;

                            case 7: // Kongol
                                Emulator.Battle.CharacterTable[slot].DF = (ushort) (Emulator.Battle.CharacterTable[slot].DF + 88 - 127);
                                Emulator.Battle.CharacterTable[slot].MDF = (ushort) (Emulator.Battle.CharacterTable[slot].MDF + 23);
                                break;
                        }
                        Emulator.Battle.CharacterTable[slot].OG_DF = Emulator.Battle.CharacterTable[slot].DF;
                        Emulator.Battle.CharacterTable[slot].OG_MDF = Emulator.Battle.CharacterTable[slot].MDF;
                        armorOfLegendSlot |= (byte) (1 << slot);
                        break;
                }

                if (Emulator.Battle.CharacterTable[slot].Shoes == 174) { //Soa's Greaves
                    for (int i = 0; i < Emulator.Battle.CharacterTable.Length; i++) {
                        if (i != slot) {
                            ushort spd = (ushort) (Emulator.Battle.CharacterTable[i].SPD - 25);
                            Emulator.Battle.CharacterTable[i].SPD = spd;
                            Emulator.Battle.CharacterTable[i].OG_SPD = spd;
                            Emulator.Memory.SecondaryCharacterTable[Globals.PARTY_SLOT[slot]].BodySPD = (byte) (Emulator.Memory.SecondaryCharacterTable[Globals.PARTY_SLOT[slot]].BodySPD - 25);
                        }
                    }
                }

                if (Globals.PARTY_SLOT[slot] == 7) {
                    ushort equipSPD = (ushort) Math.Round((double) Emulator.Memory.SecondaryCharacterTable[7].EquipSPD / 2);
                    Emulator.Memory.SecondaryCharacterTable[7].EquipSPD = equipSPD;
                    ushort spd = (ushort) (equipSPD + Emulator.Memory.SecondaryCharacterTable[7].BodySPD);
                    Emulator.Battle.CharacterTable[slot].SPD = spd;
                    Emulator.Battle.CharacterTable[slot].OG_SPD = spd;
                }
            }
        }
        ...
        public static void BattleDragoonRun(ushort encounter, bool reverseDBS, byte eleBombTurns, byte eleBombElement) {
            ...
            for (byte slot = 0; slot < Emulator.Battle.CharacterTable.Length; slot++) {
                ...
                switch (Emulator.Memory.PartySlot[slot]) {
                    case 0:
        ...
        static void DartDragoonRun(byte dragoonSpecialAttack, byte slot, double multi) {
            if (dragoonSpecialAttack == 0 || dragoonSpecialAttack == 9) {
                if ((Globals.DRAGOON_SPIRITS & 128) == 128) {
                    Emulator.Battle.CharacterTable[slot].DAT = (ushort) (dartDivineSpecialDAT * multi);
                } else {
                    ...
                    Emulator.Battle.CharacterTable[slot].DAT = (ushort) (dartDAT * multi);
                }
            } else {
                if ((Globals.DRAGOON_SPIRITS & 128) == 128) {
                    Emulator.Battle.CharacterTable[slot].DAT = (ushort) (dartDivineDAT * multi);
                } else {
                    if (Globals.CheckDMScript("btwDivineRed")) {
                        Emulator.Battle.CharacterTable[slot].DAT = (ushort) (dartDREDAT * multi);
                    } else {
                        if (burnStackSelected) {
                            multi *= 1 + (dartBurnStacks * damagePerBurn * (dartBurnStacks == dartMaxBurnStacks ? burnStackMaxAdditionMulti : burnStackAdditionMulti));
                        }
            ...
            if (currentMP[slot] < previousMP[slot]) {
                byte spell = Emulator.Battle.CharacterTable[slot].Spell_Cast;
                if (Globals.CheckDMScript("btnDivineRed")) {
                    if (spell == 1) {
                        Emulator.Battle.CharacterTable[slot].DMAT = (ushort) (explosionDREDMAT * multi);
                    } else {
                        Emulator.Battle.CharacterTable[slot].DMAT = (ushort) (finalBurstDREMAT * multi);
                    }
                } else {
                    if (spell == 0) {
                        Emulator.Battle.CharacterTable[slot].DMAT = (ushort) (flameshotDMAT * multi);
                        AddBurnStacks(burnStackFlameshot, slot);
                    } else if (spell == 1) {
                        Emulator.Battle.CharacterTable[slot].DMAT = (ushort) (explosionDMAT * multi);
                        AddBurnStacks(burnStackExplosion, slot);
                    } else if (spell == 2) {
                        Emulator.Battle.CharacterTable[slot].DMAT = (ushort) (finalBurstDMAT * multi);
                        AddBurnStacks(burnStackFinalBurst, slot);
                    } else if (spell == 3) {
                        Emulator.Battle.CharacterTable[slot].DMAT = (ushort) (redEyeDMAT * multi);
                        AddBurnStacks(burnStackRedEye, slot);
                    } else if (spell == 4 || spell == 9) {
                        Emulator.Battle.CharacterTable[slot].DMAT = (ushort) (divineDMAT * multi);
                    }
                }
                previousMP[slot] = currentMP[slot];
                Emulator.Battle.CharacterTable[slot].Spell_Cast = 255;
            } else {
                if (currentMP[slot] > previousMP[slot]) {
                    previousMP[slot] = currentMP[slot];
                }
        ...
        static void LavitzDragoonRun(byte dragoonSpecialAttack, byte slot, double multi) {
            ...
            if (dragoonSpecialAttack == 1 || dragoonSpecialAttack == 5) {
                Emulator.Battle.CharacterTable[slot].DAT = (ushort) (lavitzSpecialDAT * multi);
            } else {
                Emulator.Battle.CharacterTable[slot].DAT = (ushort) (lavitzDAT * multi);
            }
            ...
            if (currentMP[slot] != previousMP[slot] && currentMP[slot] < previousMP[slot]) {
                byte spell = Emulator.Battle.CharacterTable[slot].Spell_Cast;
                if (spell == 5 || spell == 14) {
                    Emulator.Battle.CharacterTable[slot].DMAT = (ushort) (wingBlasterDMAT * multi);
                    checkWingBlaster = true;
                    for (int i = 0; i < Globals.MONSTER_SIZE; i++) trackMonsterHP[i] = Emulator.Battle.MonsterTable[i].HP;
                } else if (spell == 6 || spell == 17) {
                    Emulator.Battle.CharacterTable[slot].DMAT = (ushort) (gasplessDMAT * multi);
                    checkGaspless = true;
                    for (int i = 0; i < Globals.MONSTER_SIZE; i++) trackMonsterHP[i] = Emulator.Battle.MonsterTable[i].HP;
                } else if (spell == 8) {
                    Emulator.Battle.CharacterTable[slot].DMAT = (ushort) (jadeDragonDMAT * multi);
                    checkJadeDragon = true;
                    for (int i = 0; i < Globals.MONSTER_SIZE; i++) trackMonsterHP[i] = Emulator.Battle.MonsterTable[i].HP;
                } else if (Globals.DIFFICULTY_MODE.Contains("Hell") && (spell == 7 || spell == 26)) {
                    checkFlowerStorm = true;
                    for (int x = 0; x < 3; x++) {
                        if (Emulator.Battle.CharacterTable[x].HP > 0) {
                            Emulator.Battle.CharacterTable[x].PWR_DF_Turn = 0;
                            Emulator.Battle.CharacterTable[x].PWR_MDF_Turn = 0;
                        }
                    }
                }
                previousMP[slot] = currentMP[slot];
                ...
                if (checkWingBlaster || checkGaspless || checkJadeDragon) {
                    for (int i = 0; i < Globals.MONSTER_SIZE; i++) {
                        if (Emulator.Battle.MonsterTable[i].HP < trackMonsterHP[i]) {
                            double tpDamage = checkWingBlaster ? wingBlasterTPDamage : checkGaspless ? gasplessTPDamage : jadeDragonTPDamage;
                            if (Emulator.Battle.MonsterTable[i].Speed_Down_Turn > 0) tpDamage /= 3;
                            Emulator.Battle.MonsterTable[i].Turn = (ushort) Math.Max(0, Emulator.Battle.MonsterTable[i].Turn - tpDamage);
                            checkWingBlaster = checkGaspless = checkJadeDragon = false;
                        }
                    }

                    if (Emulator.Battle.CharacterTable[slot].Action != 10 && Emulator.Battle.CharacterTable[slot].Action != 26) {
                        checkWingBlaster = checkGaspless = checkJadeDragon = false;
                    }
                }
                ...
                if (checkFlowerStorm) {
                    ...
                    for (int x = 0; x < 3; x++) {
                        if (Globals.PARTY_SLOT[x] < 9) {
                            if (Emulator.Battle.CharacterTable[x].PWR_DF_Turn != 0) {
                                changed = true;
                            }
                        }
                    }
                    ...
                    if (changed) {
                        for (int x = 0; x < 3; x++) {
                            if (Globals.PARTY_SLOT[x] == 1 || Globals.PARTY_SLOT[x] == 5) {
                                Emulator.Battle.CharacterTable[x].PWR_DF_Turn = (byte) (flowerStorm + 1);
                                Emulator.Battle.CharacterTable[x].PWR_MDF_Turn = (byte) (flowerStorm + 1);
                            } else {
                                Emulator.Battle.CharacterTable[x].PWR_DF_Turn = flowerStorm;
                                Emulator.Battle.CharacterTable[x].PWR_MDF_Turn = flowerStorm;
                            }
                        }
                        checkFlowerStorm = false;
                    }
        ...
        static void ShanaDragoonRun(byte dragoonSpecialAttack, byte slot, double multi) {
            if (starChildren > 0) {
                if (starChildren == 3 && Emulator.Battle.CharacterTable[slot].Action == 0)
                    starChildren = 2;
                if (starChildren == 2 && Emulator.Battle.CharacterTable[slot].Action == 8)
                    starChildren = 1;
                if (starChildren == 1 && Emulator.Battle.CharacterTable[slot].Action != 8) {
                    starChildren = 0;
                    Emulator.Battle.CharacterTable[slot].HP_Regen = recoveryRateSave;
                }
            }

            if (slot == 0 && Emulator.Battle.CharacterTable[slot].Action == 10) {
                if (dragoonSpecialAttack == 2 || dragoonSpecialAttack == 8) {
                    Emulator.Battle.CharacterTable[slot].AT = (ushort) Math.Floor(shanaSaveAT * (shanaSpecialDAT / 100D));
                } else {
                    Emulator.Battle.CharacterTable[slot].AT = (ushort) Math.Floor(shanaSaveAT * (shanaDAT / 100D));
                }
            } else if (slot == 0 && Emulator.Battle.CharacterTable[slot].Action == 8) {
                Emulator.Battle.CharacterTable[slot].AT = shanaSaveAT;
            }
            ...
            if (dragoonSpecialAttack == 2 || dragoonSpecialAttack == 8) {
                Emulator.Battle.CharacterTable[slot].DAT = (ushort) (shanaSpecialDAT * multi);
            } else {
                Emulator.Battle.CharacterTable[slot].DAT = (ushort) (shanaDAT * multi);
            }
            ...
            if (currentMP[slot] < previousMP[slot]) {
                ...
                if (spell == 10 || spell == 65) {
                    Emulator.Battle.CharacterTable[slot].DMAT = (ushort) (starChildrenDMAT * multi);
                    Emulator.Battle.CharacterTable[slot].HP_Regen = 100;
                    starChildren = 3;
                } else if (spell == 13) {
                    Emulator.Battle.CharacterTable[slot].DMAT = (ushort) (wSilverDragonDMAT * multi);
                }
                previousMP[slot] = currentMP[slot];
                Emulator.Battle.CharacterTable[slot].Spell_Cast = 255;
            } else {
        ...
        static void RoseDragoonRun(byte dragoonSpecialAttack, byte slot, double multi) {
            ...
            if (dragoonSpecialAttack == 3) {
                Emulator.Battle.CharacterTable[slot].DAT = (ushort) (roseSpecialDAT * multi);
            } else {
                Emulator.Battle.CharacterTable[slot].DAT = (ushort) (roseDAT * multi);
            }
            ...
            if (currentMP[slot] < previousMP[slot]) {
                ...
                if (roseEnhanceDragoon) {
                    if (spell == 15) {
                        Emulator.Battle.CharacterTable[slot].DMAT = (ushort) (enhancedAstralDrainDMAT * multi);
                        for (int x = 0; x < 3; x++) {
                            if (Globals.PARTY_SLOT[x] < 9 && Emulator.Battle.CharacterTable[slot].HP > 0) {
                                Emulator.Battle.CharacterTable[x].HP = (ushort) Math.Min(Emulator.Battle.CharacterTable[x].Max_HP, Emulator.Battle.CharacterTable[x].HP + Math.Round(Emulator.Battle.CharacterTable[slot].HP * (Emulator.ReadByte("ROSE_DRAGOON_LEVEL") * 0.04)));
                            }
                        }
                    } else if (spell == 16) {
                        Emulator.Battle.CharacterTable[slot].DMAT = (ushort) (enhancedDeathDimensionDMAT * multi);
                    } else if (spell == 19) {
                        Emulator.Battle.CharacterTable[slot].DMAT = (ushort) (enhancedDarkDragonDMAT * multi);
                        checkRoseDamage = true;
                        checkRoseDamageSave = Emulator.ReadUShort("DAMAGE_SLOT1");
                    }
                } else {
                    if (spell == 15) {
                        Emulator.Battle.CharacterTable[slot].DMAT = (ushort) (astralDrainDMAT * multi);
                        for (int x = 0; x < 3; x++) {
                            if (Globals.PARTY_SLOT[x] < 9 && Emulator.Battle.CharacterTable[slot].HP > 0) {
                                Emulator.Battle.CharacterTable[x].HP = (ushort) Math.Min(Emulator.Battle.CharacterTable[x].Max_HP, Emulator.Battle.CharacterTable[x].HP + Math.Round(Emulator.Battle.CharacterTable[slot].HP * (Emulator.ReadByte("ROSE_DRAGOON_LEVEL") * 0.05)));
                            }
                        }
                    } else if (spell == 16) {
                        Emulator.Battle.CharacterTable[slot].DMAT = (ushort) (deathDimensionDMAT * multi);
                    } else if (spell == 19) {
                        Emulator.Battle.CharacterTable[slot].DMAT = (ushort) (darkDragonDMAT * multi);
                        checkRoseDamage = true;
                        checkRoseDamageSave = Emulator.ReadUShort("DAMAGE_SLOT1");
                    }
                }
                ...
                if (currentMP[slot] > previousMP[slot]) {
                    ...
                    if (checkRoseDamage && Emulator.ReadUShort("DAMAGE_SLOT1") != checkRoseDamageSave) {
                        checkRoseDamage = false;
                        if (roseEnhanceDragoon) {
                            Emulator.Battle.CharacterTable[slot].HP = (ushort) Math.Min(Emulator.Battle.CharacterTable[slot].HP + (Emulator.ReadUShort("DAMAGE_SLOT1") * 0.4), Emulator.Battle.CharacterTable[slot].Max_HP);
                        } else {
                            Emulator.Battle.CharacterTable[slot].HP = (ushort) Math.Min(Emulator.Battle.CharacterTable[slot].HP + (Emulator.ReadUShort("DAMAGE_SLOT1") * 0.1), Emulator.Battle.CharacterTable[slot].Max_HP);
                        }
                    }
                }
            }
        ...
        static void HaschelDragoonRun(byte dragoonSpecialAttack, byte slot, double multi, byte eleBombTurns, byte eleBombElement) {
            if (dragoonSpecialAttack == 4) {
                Emulator.Battle.CharacterTable[slot].DAT = (ushort) (haschelSpecialDAT * multi);
            } else {
                Emulator.Battle.CharacterTable[slot].DAT = (ushort) (haschelDAT * multi);
            }
            ...
            if (currentMP[slot] < previousMP[slot]) {
                byte spell = Emulator.Battle.CharacterTable[slot].Spell_Cast;
                if (spell == 20) {
                    Emulator.Battle.CharacterTable[slot].DMAT = (ushort) (atomicMindDMAT * multi);
                } else if (spell == 21) {
                    Emulator.Battle.CharacterTable[slot].DMAT = (ushort) (thunderKidDMAT * multi);
                } else if (spell == 22) {
                    Emulator.Battle.CharacterTable[slot].DMAT = (ushort) (thunderGodDMAT * multi);
                } else if (spell == 23) {
                    Emulator.Battle.CharacterTable[slot].DMAT = (ushort) (violetDragonDMAT * multi);
                }
                previousMP[slot] = currentMP[slot];
                Emulator.Battle.CharacterTable[slot].Spell_Cast = 255;
            } else {
        ...
        static void MeruDragoonRun(byte dragoonSpecialAttack, byte slot, double multi) {
            if (dragoonSpecialAttack == 6) {
                Emulator.Battle.CharacterTable[slot].DAT = (ushort) (meruSpecialDAT * multi);
            } else {
                Emulator.Battle.CharacterTable[slot].DAT = (ushort) (meruDAT * multi);
            }
            ...
            if (currentMP[slot] < previousMP[slot]) {
                ...
                if (meruEnhanceDragoon) {
                    if (spell == 24) {
                        Emulator.Battle.CharacterTable[slot].DMAT = (ushort) (enhancedFreezingRingDMAT * multi);
                    } else if (spell == 25) {
                        trackRainbowBreath = true;
                    } else if (spell == 27) {
                        Emulator.Battle.CharacterTable[slot].DMAT = (ushort) (enhancedDiamondDustDMAT * multi);
                    } else if (spell == 28) {
                        Emulator.Battle.CharacterTable[slot].DMAT = (ushort) (enhancedBlueSeaDragonDMAT * multi);
                    }
                } else {
                    if (spell == 24) {
                        Emulator.Battle.CharacterTable[slot].DMAT = (ushort) (freezingRingDMAT * multi);
                    } else if (spell == 27) {
                        Emulator.Battle.CharacterTable[slot].DMAT = (ushort) (diamondDustDMAT * multi);
                    } else if (spell == 28) {
                        Emulator.Battle.CharacterTable[slot].DMAT = (ushort) (blueSeaDragonDMAT * multi);
                    }
                }
                previousMP[slot] = currentMP[slot];
                Emulator.Battle.CharacterTable[slot].Spell_Cast = 255;
                ...
                if (currentMP[slot] > previousMP[slot]) {
                    ...
                    if (trackRainbowBreath) {
                        if (Emulator.Battle.CharacterTable[slot].Action == 2 || Emulator.Battle.CharacterTable[slot].Action == 9) {
                            for (int x = 0; x < 3; x++) {
                                if (Globals.PARTY_SLOT[slot] < 9) {
                                    Emulator.Battle.CharacterTable[x].HP = (ushort) Math.Min(short.MaxValue, Math.Round(Emulator.Battle.CharacterTable[x].HP * 1.65));
                                }
                            }
                            trackRainbowBreath = false;
                        }
        ...
        static void KongolDragoonRun(byte dragoonSpecialAttack, byte slot, double multi) {
            if (dragoonSpecialAttack == 7) {
                Emulator.Battle.CharacterTable[slot].DAT = (ushort) (kongolSpecialDAT * multi);
            } else {
                Emulator.Battle.CharacterTable[slot].DAT = (ushort) (kongolDAT * multi);
            }
            ...
            if (currentMP[slot] < previousMP[slot]) {
                byte spell = Emulator.Battle.CharacterTable[slot].Spell_Cast;
                if (spell == 29) {
                    Emulator.Battle.CharacterTable[slot].DMAT = (ushort) (grandStreamDMAT * multi);
                } else if (spell == 30) {
                    Emulator.Battle.CharacterTable[slot].DMAT = (ushort) (meteorStrike * multi);
                } else if (spell == 31) {
                    Emulator.Battle.CharacterTable[slot].DMAT = (ushort) (goldDragon * multi);
                }
                previousMP[slot] = currentMP[slot];
                Emulator.Battle.CharacterTable[slot].Spell_Cast = 255;
            } else {
        ...
        static void SetupBurnStacks() {
            for (int slot = 0; slot < Emulator.Battle.CharacterTable.Length; slot++) {
                if (Globals.PARTY_SLOT[slot] == 0) {
                    int dlv = Emulator.Battle.CharacterTable[slot].DLV;
                ...
                if (Globals.PARTY_SLOT[i] == 0) {
                    if (resetBurnStack) {
                        if (Emulator.Battle.CharacterTable[i].Action == 8 || Emulator.Battle.CharacterTable[i].Action == 10) {
                            ...
                            Emulator.Battle.CharacterTable[i].MP = 0;
                            Emulator.Battle.CharacterTable[i].Spell_Cast = 255;
                            burnStackSelected = false;
                            resetBurnStack = false;
                            Constants.WriteGLogOutput("Dart Burn Stacks: " + dartBurnStacks + " / " + dartMaxBurnStacks);
                        }
                    ...
                    if (Emulator.Battle.CharacterTable[i].Action == 10 && dartPreviousAction[i] != 10) {
                        dartBurnMP[i] = Emulator.Battle.CharacterTable[i].MP;
                    }

                    if (dartPreviousAction[i] == 26 && dartBurnMPHeal) {
                        if (dartBurnStacks >= 4 && dartPreviousBurnStacks < 4) {
                            Emulator.Battle.CharacterTable[i].MP = (ushort) Math.Min(Emulator.Battle.CharacterTable[i].MP + 10, Emulator.Battle.CharacterTable[i].Max_MP);
                        } else if (dartBurnStacks >= 8 && dartPreviousBurnStacks < 8) {
                            Emulator.Battle.CharacterTable[i].MP = (ushort) Math.Min(Emulator.Battle.CharacterTable[i].MP + 20, Emulator.Battle.CharacterTable[i].Max_MP);
                        } else if (dartBurnStacks >= 12 && dartPreviousBurnStacks < 12) {
                            Emulator.Battle.CharacterTable[i].MP = (ushort) Math.Min(Emulator.Battle.CharacterTable[i].MP + 30, Emulator.Battle.CharacterTable[i].Max_MP);
                        }
                        dartBurnMPHeal = false;
                    }
                    ...
                    if (dartBurnStacks > 0) {
                        if (Emulator.Battle.CharacterTable[i].Action == 10) {
                            byte menu = Emulator.Battle.CharacterTable[i].Menu;

                            if (menu == 96 || menu == 98) {
                                byte[] icons = { 1, 4, 9, 3, 3 };
                                Emulator.Battle.CharacterTable[i].Menu = (byte) (225 - (96 - Emulator.Battle.CharacterTable[i].Menu));
                                Thread.Sleep(60);
                        ...
                        } else if (Emulator.Battle.CharacterTable[i].Action == 26) {
                            if (dartPreviousAction[i] == 10 && dartBurnMP[i] == Emulator.Battle.CharacterTable[i].MP) {
                                byte dlv = Emulator.Battle.CharacterTable[i].DLV;
                        ...
                        if (Emulator.Battle.CharacterTable[i].Action == 26) {
                            if (dartPreviousAction[i] == 10 && dartBurnMP[i] == Emulator.Battle.CharacterTable[i].MP) {
                                byte dlv = Emulator.Battle.CharacterTable[i].DLV;
                                AddBurnStacks(dlv == 5 ? 2 : 1, i);
                            }
                    ...
                    dartPreviousAction[i] = Emulator.Battle.CharacterTable[i].Action;
                }
            }
        } // TODO rewrite
        ...
        static void NoEscape() { // TODO remove Emulator
            if (Globals.CheckDMScript("btnBlackRoom")) {
                var map = Emulator.Memory.MapID;
                if (((map >= 5 && map <= 7) || (map >= 624 && map <= 625))) {
                    return;
                }
                for (int i = 0; i < Emulator.Battle.CharacterTable.Length; i++) {
                    byte noEscape = Emulator.ReadByte("NO_ESCAPE", (Emulator.Battle.MonsterTable.Length + i) * 0x20);
                    noEscape |= 8;
                    Emulator.WriteByte("NO_ESCAPE", noEscape, (Emulator.Battle.MonsterTable.Length + i) * 0x20);
                }
            }
        }
        ...
        public static void EquipChangesRun(int inventorySize) {
            for (int slot = 0; slot < Emulator.Battle.CharacterTable.Length; slot++) {
                if ((spiritEaterSlot & ( 1 << slot)) != 0) { // If Else performance?
                    if (Emulator.Battle.CharacterTable[slot].SP == Emulator.Battle.CharacterTable[slot].Max_MP) {
                        Emulator.Battle.CharacterTable[slot].SP_Regen -= Globals.DICTIONARY.ItemList[159].Special_Ammount; // TODO LoDDictionary 
                        spiritEaterCheck = true;
                    } else if (spiritEaterCheck) {
                        Emulator.Battle.CharacterTable[slot].SP_Regen = Emulator.Memory.SecondaryCharacterTable[Globals.PARTY_SLOT[slot]].SP_Regen;
                        spiritEaterCheck = false;
                    }

                } else if ((harpoonSlot & (1 << slot)) != 0) {
                    var sp = Emulator.Battle.CharacterTable[slot].SP;
                    if (sp >= 400 && Emulator.Battle.CharacterTable[slot].Action == 10) {
                        if (sp == 500) {
                            Emulator.WriteAoB(Globals.C_POINT - 0x388 * slot + 0xC0, "00 00 00 04"); // TODO
                            Emulator.Battle.CharacterTable[slot].SP = 200;
                            Emulator.Battle.CharacterTable[slot].DragoonTurns = 2;
                        } else {
                    ...
                    if (harpoonCheck && Emulator.Battle.CharacterTable[slot].DragoonTurns == 0) {
                        harpoonCheck = false;
                    }

                } else if ((elementArrowSlot & (1 << slot)) != 0) {
                    var currentAction = Emulator.Battle.CharacterTable[slot].Action;
                    if (elementArrowLastAction != currentAction) {
                        ...
                        if (elementArrowLastAction == 8) {
                            ...
                            if (elementArrowLastAction == 10) {
                                Emulator.Battle.CharacterTable[slot].Weapon_Element = 0;
                            }
                    ...
                    if (Emulator.Battle.CharacterTable[slot].Action == 136) {
                        var damageSlot = Emulator.ReadUShort("DAMAGE_SLOT1"); // TODO
                        if (damageSlot != 0) {
                            ushort HP = Emulator.Battle.CharacterTable[slot].HP;
                            Emulator.Battle.CharacterTable[slot].HP = (ushort) Math.Min(HP + Math.Round(damageSlot * 0.02) + 2, HP);
                            Emulator.WriteUShort("DAMAGE_SLOT", 0); // TODO
                        }
                    }

                } else if ((batteryGloveSlot & (1 << slot)) != 0) {
                    if ((Emulator.Battle.CharacterTable[slot].Action == 136 || Emulator.Battle.CharacterTable[slot].Action == 26) && (batteryGloveLastAction != 136 || batteryGloveLastAction != 26)) {
                        batteryGloveCharge++;
                        if (batteryGloveCharge == 7) {
                            Emulator.Battle.CharacterTable[slot].AT = (ushort) Math.Round(Emulator.Battle.CharacterTable[slot].AT * 2.5);
                        } else if (batteryGloveCharge > 7) {
                            batteryGloveCharge = 1;
                            Emulator.Battle.CharacterTable[slot].AT = Emulator.Battle.CharacterTable[slot].OG_AT;
                        }

                        batteryGloveLastAction = Emulator.Battle.CharacterTable[slot].Action;
                    }

                } else if ((giantAxeSlot & (1 << slot)) != 0) {
                    var action = Emulator.Battle.CharacterTable[slot].Action;
                    if (action == 136 && giantAxeLastAction != action) {
                        if (new Random().Next(0, 9) < 2) {
                            Emulator.Battle.CharacterTable[slot].Guard = 1;
                        }
                    }
                ...
                if ((fakeLegendCasqueSlot & (1 << slot)) != 0) {
                    if (fakeLegendCasqueCheck[slot] && Emulator.Battle.CharacterTable[slot].Guard == 1) {
                        if (new Random().Next(0, 9) < 3) {
                            Emulator.Battle.CharacterTable[slot].MDF += 40;
                        }
                        fakeLegendCasqueCheck[slot] = false;
                    }
                    if (!fakeLegendCasqueCheck[slot] && ((Emulator.Battle.CharacterTable[slot].Action & 8) != 0)) {
                        Emulator.Battle.CharacterTable[slot].MDF = Emulator.Battle.CharacterTable[slot].OG_MDF;
                        fakeLegendCasqueCheck[slot] = true;
                    }

                } else if ((legendCasqueSlot & (1 << slot)) != 0) {
                    if (legendCasqueCheckMDF[slot] && Emulator.Battle.CharacterTable[slot].Guard == 1) {
                        if (legendCasqueCount[slot] >= 3) {
                            Emulator.Battle.CharacterTable[slot].MDF = (ushort) Math.Ceiling(Emulator.Battle.CharacterTable[slot].MDF * 1.2);
                    ...
                    if (legendCasqueCheckShield[slot] && Emulator.Battle.CharacterTable[slot].Guard == 0 && (Emulator.Battle.CharacterTable[slot].Action & 8) == 0) {
                        if (new Random().Next(0, 9) < 1) {
                            Emulator.Battle.CharacterTable[slot].MagicalShield = Math.Max((byte) 1, Emulator.Battle.CharacterTable[slot].MagicalShield);
                        }
                        legendCasqueCheckShield[slot] = false;
                    }
                    if ((legendCasqueCheckMDF[slot] || legendCasqueCheckShield[slot]) && (Emulator.Battle.CharacterTable[slot].Action & 8) != 0) {
                ...
                if ((fakeLegendArmorSlot & (1 << slot)) != 0) {
                    if (fakeLegendArmorCheck[slot] && Emulator.Battle.CharacterTable[slot].Guard == 1) {
                        if (new Random().Next(0, 9) < 3) {
                            Emulator.Battle.CharacterTable[slot].DF += 40;
                        }
                        fakeLegendArmorCheck[slot] = false;
                    }
                    if (!fakeLegendArmorCheck[slot] && ((Emulator.Battle.CharacterTable[slot].Action & 8) != 0)) {
                        Emulator.Battle.CharacterTable[slot].DF = Emulator.Battle.CharacterTable[slot].OG_DF;
                        fakeLegendArmorCheck[slot] = true;
                    }

                } else if ((armorOfLegendSlot & (1 << slot)) != 0) {
                    if (armorOfLegendCheckDF[slot] && Emulator.Battle.CharacterTable[slot].Guard == 1) {
                        if (armorOfLegendCount[slot] >= 3) {
                            Emulator.Battle.CharacterTable[slot].DF = (ushort) Math.Ceiling(Emulator.Battle.CharacterTable[slot].DF * 1.2);
                    ...
                    if (armorOfLegendCheckShield[slot] && Emulator.Battle.CharacterTable[slot].Guard == 0 && (Emulator.Battle.CharacterTable[slot].Action & 8) == 0) {
                        if (new Random().Next(0, 9) < 1) {
                            Emulator.Battle.CharacterTable[slot].PhysicalShield = Math.Max((byte) 1, Emulator.Battle.CharacterTable[slot].PhysicalShield);
                        }
                        armorOfLegendCheckShield[slot] = false;
                    }
                    if ((armorOfLegendCheckDF[slot] || armorOfLegendCheckShield[slot]) && (Emulator.Battle.CharacterTable[slot].Action & 8) != 0) {
                ...
                if ((soasAnkhSlot & (1 << slot)) != 0) {
                    ...
                    int kill = -1;
                    int lastPartyID = -1;
                    if (Emulator.Battle.CharacterTable[slot].HP == 0) {
                        for (int x = 0; x < 3; x++) {
                            if (x != slot && Globals.PARTY_SLOT[slot] < 9 && Emulator.Battle.CharacterTable[x].HP > 0) {
                                alive = true;
                            }
                        }
                        ...
                        if (alive) {
                            for (int x = 0; x < 3; x++) {
                                if (kill == -1 && new Random().Next(0, 9) < 5 && Emulator.Battle.CharacterTable[x].HP > 0) {
                                    kill = x;
                                } else {
                                    lastPartyID = x;
                                }
                        ...
                        if (kill != -1) {
                            Emulator.Battle.CharacterTable[kill].HP = 0;
                            Emulator.Battle.CharacterTable[kill].Action = 192;
                        } else {
                            Emulator.Battle.CharacterTable[lastPartyID].HP = 0;
                            Emulator.Battle.CharacterTable[lastPartyID].Action = 192;
                        }
                        Emulator.Battle.CharacterTable[slot].HP = 1;
                    } else {
                        Emulator.Battle.CharacterTable[slot].Max_HP = 0;
                        Emulator.Battle.CharacterTable[slot].Revive = 0;
                        Emulator.Battle.CharacterTable[slot].Action = 192;
                    }
                }
            }
        }
        ...
        public static void MagicInventoryHandler() {
            for (int i = 0; i < 3; i++) {
                if (Globals.PARTY_SLOT[i] == 2 || Globals.PARTY_SLOT[i] == 8) {
                    ...
                    /*if (dartBurnStacks > 0) {
                    if (Emulator.Battle.CharacterTable[i].Read("Action") == 10) {
                        int menu = Emulator.Battle.CharacterTable[i].Read("Menu");

                        if (menu == 96 || menu == 98) {
                            byte[] icons = { 1, 4, 9, 3, 3 };
                            Emulator.Battle.CharacterTable[i].Write("Menu", 225 - (96 - Emulator.Battle.CharacterTable[i].Read("Menu")));
                            Thread.Sleep(60);
                            for (int x = menu == 96 ? 1 : 0; x < icons.Length; x++) {
                                Emulator.WriteByte(Globals.M_POINT + 0xD34 + (x - (menu == 96 ? 1 : 0)) * 0x2, icons[x]);
                            }
